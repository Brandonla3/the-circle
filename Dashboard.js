...
export default function Dashboard() {
  ...
  const fetchScores = useCallback(async (silent = false) => {
    ...
    setError(null);
    try {
      const r = await fetch(`/api/scores?date=${fmtDate(date)}`);
  ...
  const fetchGameDetail = async (eventId) => {
    setGameDetail(null);
    try {
      const r = await fetch(`/api/summary?event=${eventId}`);
  ...
  return (
    <div className="min-h-screen w-full" style={{ background: '#0a0908' }}>
          ...
          <div>
            <div className="flex items-center justify-between mb-8 flex-wrap gap-4">
              <div className="flex items-center gap-2">
                <button onClick={() => shiftDate(-1)} className="p-2 rounded-full border border-white/10 hover:border-white/30 text-white/60 hover:text-white transition">
                ...
                <button onClick={() => setDate(new Date())} className="ml-2 px-3 py-1.5 text-[10px] mono uppercase tracking-widest rounded-full border border-white/10 text-white/60 hover:text-white hover:border-white/30 transition">
                  Today
                </button>
              </div>
      ...
      {selectedGame && (
        <GameModal game={selectedGame} detail={gameDetail} onClose={() => { setSelectedGame(null); setGameDetail(null); }} />
      )}
...
function GameCard({ game, index, onClick }) {
  ...
  const TeamRow = ({ team, side }) => {
    ...
    return (
      <div className={`flex items-center justify-between py-2 ${dim ? 'opacity-40' : ''}`}>
        ...
        <div className={`mono text-2xl font-bold tabular-nums ${winner === side ? 'text-white' : 'text-white/70'}`}>
          {team?.score ?? '—'}
        </div>
      </div>
    );
  ...
  return (
    <div onClick={onClick} className="card-enter relative rounded-xl border border-white/10 bg-gradient-to-br from-white/[0.03] to-transparent p-5 cursor-pointer hover:border-white/30 hover:from-white/[0.06] transition-all" style={{ animationDelay: `${Math.min(index * 40, 400)}ms` }}>
      <div className="flex items-center justify-between mb-3">
        ...
        {comp.broadcasts?.[0]?.names?.[0] && (
          <span className="text-[9px] mono uppercase text-white/30">{comp.broadcasts[0].names[0]}</span>
        )}
      </div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm" onClick={onClose}>
      <div className="relative w-full max-w-3xl max-h-[90vh] overflow-y-auto rounded-2xl border border-white/10 p-6" style={{ background: '#141210' }} onClick={(e) => e.stopPropagation()}>
        <button onClick={onClose} className="absolute top-4 right-4 p-2 rounded-full hover:bg-white/10 text-white/60 hover:text-white">
          <X className="h-4 w-4" />
        </button>
            ...
            <tbody>
              {[away, home].map((t, ti) => (
                <tr key={ti} className="border-t border-white/5">
                  ...
                  {innings.map((i) => {
                    const ls = t?.linescores?.[i - 1];
                    return <td key={i} className="px-2 text-center text-white/70">{ls?.value ?? ''}</td>;
                  })}
